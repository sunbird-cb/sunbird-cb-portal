...
export class KnowledgeResourceService {
  ...
  countFileType(fileList: NSKnowledgeResource.IKrFiles[], fType: string) {
      let count = 0
      if(fileList && fileList.length > 0 && fileType.includes(fType)) {
        if(fType == 'image'){
          count = (_.filter(fileList, f => {return _.includes(imageTypes, f.fileType)}) || []).length
        } else {
          count = (_.filter(fileList, { 'fileType': fType}) || []).length
        }
