...
export class HtmlMobileComponent implements OnInit, OnDestroy {
        ...
        async data => {
          ...
          if (tempHtmlData) {
            ...
            if (!this.alreadyRaised && this.oldData) {
              // this.raiseEvent(WsEvents.EnumTelemetrySubType.Unloaded, this.oldData)      //mobile requirement telemetry disabled
      ...
      this.routeDataSubscription = this.activatedRoute.data.subscribe(
        async data => {
          ...
          if (this.alreadyRaised && tempHtmlData) {
            // this.raiseEvent(WsEvents.EnumTelemetrySubType.Unloaded, tempHtmlData) //mobile requirement telemetry disabled
            if (!this.hasFiredRealTimeProgress) {
              // this.fireRealTimeProgress()
              if (this.realTimeProgressTimer) {
                clearTimeout(this.realTimeProgressTimer)
  ...
  async ngOnDestroy() {
    ...
    // if (this.htmlData) {
    //   this.raiseEvent(WsEvents.EnumTelemetrySubType.Unloaded, this.htmlData)      //mobile requirement telemetry disabled
    // }
